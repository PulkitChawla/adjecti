...
public class ActionRequestServlet extends HttpServlet {
	...
	public void init(ServletConfig config) {
		...
		try {
			...
			for (int i = 0; i < actionNodeList.getLength(); i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {
					...
					// action.setForwardActions(eElement.getElementsByTagName("forward").item(0).getTextContent());
					...
					for (int j = 0; j < forwardNodeList.getLength(); j++) {
						...
						if (node2.getNodeType() == Node.ELEMENT_NODE) {
							...
							action.setForwardAction(forwardList);// working till here
						}
					}
		...
		// for (Action action11 : actions) {
		// System.out.println(action11.getActionName());
		// System.out.println(action11.getActionClass());
		// for (Forward forward2 : action11.getForward()) {
		// System.out.println(forward2.getPage());
		// System.out.println(forward2.getStatus());
		// }
		// }
	...
	private RequestHandler getRequestHandler(Action action) {
		try {
			Class c=Class.forName(action.getActionClass());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String page=getPageName(status,action);
	...
	public String getPageName(String status,Action action) {
		...
		for(Forward forward:action.getForwardAction()) {
			...
			} else if(status.equals("failure")) {//.equals(forward.getStatus()
				 page=forward.getPage();
			}
		}
		return page;
